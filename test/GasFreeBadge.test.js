...
describe('GasFreeBadge', function () {
  ...
  describe('claimBadge()', function () {
    it('mints to recipient and emits BadgeClaimed', async function () {
      ...
      await expect(nft.claimBadge(user1.address))
    ...
    it('allows multiple claims per wallet', async function () {
      const { nft, user1 } = await loadFixture(deployFixture);
      await nft.claimBadge(user1.address);
      await nft.claimBadge(user1.address);
    ...
    it('reverts on zero address', async function () {
      const { nft } = await loadFixture(deployFixture);
      await expect(nft.claimBadge(ethers.ZeroAddress)).to.be.revertedWithCustomError(
        nft,
        'ZeroAddress'
      );
    });
    ...
    it('reverts when paused', async function () {
      ...
      await expect(nft.claimBadge(user1.address)).to.be.revertedWithCustomError(
        nft,
        'ContractPaused'
      );
    });
